...
def audit_menu(rows):
    ...
    for r in rows:
        for e in r.get("events", []):
            if e.get("action") == "menu_violation": continue
            cid, menu = e.get("choice_id"), e.get("menu")
            if not cid or not isinstance(menu, list):
                failures += 1; continue
            hits = [m for m in menu if m.get("id") == cid]
            if len(hits) != 1:
                failures += 1; continue
            m = hits[0]
            if m.get("action") != e.get("action"): failures += 1
            mp = m.get("price") if m.get("action") == "offer" else None
            ep = e.get("price") if e.get("action") == "offer" else None
            if mp != ep: failures += 1
    return failures
...
def main():
    ...
    menu_reconstruction_failures = audit_menu(rows)
    bounds = 0
    for r in rows:
    ...
    for r in rows:
        if r["seller"] == "posted": posted[(r["product_id"], r["tightness"], r["state"])].append(r)
    posted_agreement_disagreement_groups, posted_max_price_range = 0, 0.0
    for group in posted.values():
        if len(group) != 3 or len({r.get("agreement") for r in group}) != 1:
            posted_agreement_disagreement_groups += 1
        prices = [r["final_price"] for r in group if r.get("agreement") and r.get("final_price") is not None]
        if prices: posted_max_price_range = max(posted_max_price_range, max(prices) - min(prices))
    audit = {
        "rows": len(rows), "expected": EXPECTED, "source_files": len(files), "duplicates": duplicate_count,
        "infrastructure_failures": infra, "menu_violations": menu_violations,
        "menu_reconstruction_failures": menu_reconstruction_failures, "price_constraint_failures": bounds,
        "posted_agreement_disagreement_groups": posted_agreement_disagreement_groups,
    ...
    audit["technical_gate_pass"] = (len(rows) == EXPECTED and len(files) == 9 and duplicate_count == 0 and infra == 0
                                    and menu_violations == 0 and menu_reconstruction_failures == 0 and bounds == 0
    ...
    out = {
        "experiment_id": "last-price-e02e-finite-actions-20260810", "audit": audit,
        "primary": {"P1_hidden_buyer_first_llama_minus_qwen": p1, "P2_anchor_DID_llama_minus_qwen": p2},
        "product_primary": product_primary, "agreement": agreement, "anchor_effect_by_model": anchor,
        "order_effect_by_model": order, "tightness": tight, "experienced_inflation": inf,
        "inflation_differences": infdiff,
    }
    Path("aggregate").mkdir(exist_ok=True)
    Path("aggregate/E02E_RESULTS.json").write_text(json.dumps(out, indent=2))
    Path("aggregate/E02E_AUDIT.json").write_text(json.dumps(audit, indent=2))
    lines = [
        ...
        f"Infrastructure failures: **{infra}**", f"Menu violations: **{menu_violations}**",
        f"Menu reconstruction failures: **{menu_reconstruction_failures}**", f"Realized price-constraint failures: **{bounds}**",
        f"Posted agreement-disagreement groups (reported, not a technical gate): **{posted_agreement_disagreement_groups}**",
        f"Posted conditional price max model range: **{posted_max_price_range}**", "", "## Primary", "",
    ...
    for p in PRODUCTS: lines.append(f"- {p}: P1 {product_primary[p]['P1']['mean_pp']} pp; P2 {product_primary[p]['P2']['mean_pp']} pp")
    lines += ["", "## Agreement"]
    for b in BUYERS: lines.append(f"- {b}: {agreement[b]['trades']}/{agreement[b]['episodes']} trades; mean price/reference {agreement[b]['mean_price_reference_pct']}%")
    lines += ["", "## Interpretation boundary", "Open-weight finite-action validation only. Prices are chosen from actor-private feasible menus; no free-form model-generated numeric price is executable. Commercial-model generalization remains untested."]
    Path("aggregate/E02E_REPORT.md").write_text("\n".join(lines) + "\n")
